...
export interface TaskModalProps { onClose: () => void; onEdit?: (taskId: string) => void; taskId?: string }
const formatDate = (value: string) => new Intl.DateTimeFormat(undefined, { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(value))

export function TaskModal({ onClose, onEdit, taskId }: TaskModalProps) {
  const { data: task, error, isError, isLoading, refetch } = useTask(taskId); const updateStatus = useUpdateTaskStatus(taskId ?? '', task?.projectId); const [isDeleteOpen, setIsDeleteOpen] = useState(false); const isMissing = isAxiosError(error) && error.response?.status === 404
