...
function formatRelativeTime(value: string) {
  ...
  return new Intl.RelativeTimeFormat(undefined, { numeric: "auto" }).format(
    -amount,
    `${unit}s` as Intl.RelativeTimeFormatUnit,
  );
}
  ...
  return (
    <main className="mx-auto w-full max-w-6xl space-y-8 px-6 py-10 sm:px-8">
      ...
      <section aria-labelledby="quick-actions-heading">
        <Card className="p-5">
          <div className="mb-4">
            <h2 className="font-semibold text-text" id="quick-actions-heading">
              Quick Actions
            </h2>
            <p className="mt-1 text-sm text-muted">
              Start your next piece of work.
            </p>
          </div>
          <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
            <Button
              className="justify-start"
              onClick={() => navigate("/projects?modal=create")}
            >
            ...
            <Button
              className="justify-start"
              onClick={() => navigate("/notes?modal=create")}
              variant="secondary"
            >
              <Plus aria-hidden="true" className="size-4" />
              New Note
            ...
            <Button
              className="justify-start"
              onClick={() => navigate("/snippets?modal=create")}
              variant="secondary"
            >
              <Plus aria-hidden="true" className="size-4" />
              New Snippet
      ...
      {isLoading ? (
        ...
        <>
          ...
          >
            <StatisticCard
              description={`${data.statistics.activeProjects} active`}
              icon={FolderKanban}
              label="Projects"
              value={data.statistics.projects}
            />
            <StatisticCard
              description={`${data.statistics.completedTasks} completed`}
              icon={ListTodo}
              label="Tasks"
            ...
            <StatisticCard
              ...
              icon={Code2}
              label="Snippets"
              value={data.statistics.snippets}
            />
            <StatisticCard
              description="Across all projects"
              icon={Tags}
              label="Labels"
              value={data.statistics.labels}
            />
          </section>
          <section className="grid gap-6 xl:grid-cols-2">
            <Card className="p-5">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted">
                    {data.tasksSummary.total} tasks across your workspace
                  </p>
                </div>
              ...
              <dl className="mt-5 grid gap-3 sm:grid-cols-2">
                {[
                  ...
                  <div
                    className="flex items-center justify-between rounded-lg bg-app px-3 py-2.5"
                    key={status}
                  >
                    <dt>
            ...
            <Card className="p-5">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted">
                    A quick view of your project workspace
                  </p>
                </div>
                <Button
                  onClick={() => navigate("/projects")}
              ...
              <dl className="mt-5 grid grid-cols-2 gap-3 text-sm">
                <div className="rounded-lg bg-app p-3">
                  ...
                  <dd className="mt-1 text-xl font-semibold text-text">
                    {data.projectsSummary.total}
                  </dd>
                </div>
                <div className="rounded-lg bg-app p-3">
                  <dt>
                ...
                <div className="rounded-lg bg-app p-3">
                  ...
                  <dd className="mt-1 text-xl font-semibold text-text">
                    {data.projectsSummary.favorites}
                  </dd>
                </div>
              </dl>
            </Card>
          ...
          <section className="grid gap-6 xl:grid-cols-2">
              ...
              {data.upcomingTasks.length ? (
                <Card className="divide-y divide-border">
                  {data.upcomingTasks.map((task) => (
                    <button
                      className="grid w-full gap-3 p-4 text-left transition-colors hover:bg-app focus-visible:outline-2 focus-visible:outline-primary sm:grid-cols-[minmax(0,1fr)_auto_auto] sm:items-center"
                      key={task.id}
                      onClick={() =>
                        navigate(
                          `/projects/${task.project.id}/tasks?task=${task.id}`,
                        )
                      }
                      type="button"
                    >
                      <span className="min-w-0">
                        <span className="flex items-center gap-2">
                          <span
                            aria-label={`${task.project.name} project color`}
                            className="size-2.5 shrink-0 rounded-full"
                            style={{
                              backgroundColor:
                                task.project.color ?? "var(--color-primary)",
                            }}
                          />
                ...
                <Card className="p-5">
                  <div className="flex items-center gap-3">
                    <span className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-app text-muted">
            ...
            <section aria-labelledby="recently-updated-heading">
              <div className="mb-4">
                ...
                <p className="mt-1 text-sm text-muted">
                  Your latest changes across DevLupo.
                </p>
              </div>
              <Card className="divide-y divide-border">
                {data.recentActivity.length ? (
                  data.recentActivity.map((activity) => {
                    const { icon: Icon, label } = getActivityMeta(
                      activity.type,
                    );
                    return (
                      <article
                        ...
                        key={activity.id}
                      >
                        <span className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-app text-primary">
                        ...
                        {activity.project && (
                          <span className="max-w-36 truncate text-xs text-muted">
          ...
          <section className="grid gap-6 xl:grid-cols-2">
            <div>
              <div className="mb-4">
                ...
                <p className="mt-1 text-sm text-muted">Your latest notes.</p>
              </div>
              <Card className="divide-y divide-border">
                {data.recentNotes.length ? (
                  data.recentNotes.map((note) => (
                    <button
                      className="flex w-full min-w-0 items-center gap-3 px-5 py-3.5 text-left transition-colors hover:bg-app focus-visible:outline-2 focus-visible:outline-primary"
                      key={note.id}
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="mt-0.5 block truncate text-xs text-muted">
                          {note.project?.name ?? "Personal note"} · Updated{" "}
                          {new Intl.DateTimeFormat(undefined, {
                            dateStyle: "medium",
